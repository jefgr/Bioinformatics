...
while (i<length(pbmc_3k_hie_list)){
  ...
  #original data_tmp <- data.frame(matrix(0,nrow = 2638, ncol = ncol(a$data)))
  n_cells <- ncol(GetAssayData(pbmc, assay = "RNA", layer = "counts"))
  data_tmp <- data.frame(matrix(0, nrow = n_cells, ncol = ncol(a$data)))
  colnames(data_tmp)<-colnames(a$data)
  rownames(data_tmp)<-colnames(GetAssayData(pbmc, assay = "RNA", layer = "counts"))

  for (bar_code in colnames(GetAssayData(pbmc, assay = "RNA", layer = "counts"))) {
    data_tmp[bar_code,]<-a$data[bar_code,]
  }
  data_tmp[data_tmp>2.5]=2.5
  data_tmp[data_tmp<(-2.5)]=(-2.5)
  p_tmp=Heatmap(t(as.matrix(data_tmp)), #name = paste0("mat_",as.character(i+1)), 
             column_order = cell_order, 
             row_order = rev(a$feature.order), 
             cluster_rows = FALSE,
             cluster_columns = FALSE,show_column_names = FALSE,col = PurpleAndYellow(),
             show_heatmap_legend = FALSE,
             use_raster = FALSE)
  ht_list = ht_list %v% p_tmp
  for (level_ele in levels(pbmc_3k_hie_list[[i+1]])){
    if(length(str_split(level_ele,';')[[1]])==2){
      pbmc_sub1 <- subset(x = pbmc, idents = str_split(level_ele,';')[[1]])
      pbmc_sub1 <- ScaleData(pbmc_sub1, features = rownames(pbmc_3k_hie_list[[1]]))
      pbmc.markers <- FindAllMarkers(pbmc_sub1, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
      top10 <- pbmc.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
      a<-DoHeatmap_matrix(pbmc_sub1,features = top10$gene)
      #original data_tmp <- data.frame(matrix(0,nrow = 2638, ncol = ncol(a$data)))
      data_tmp <- data.frame(matrix(0, nrow = n_cells, ncol = ncol(a$data)))
      colnames(data_tmp)<-colnames(a$data)
      rownames(data_tmp)<-colnames(GetAssayData(pbmc, assay = "RNA", layer = "counts"))
      ...
      data_tmp[data_tmp>2.5]=2.5
      data_tmp[data_tmp<(-2.5)]=(-2.5)
      p_tmp=Heatmap(t(as.matrix(data_tmp)), 
                    column_order = cell_order, 
                    row_order = rev(a$feature.order), 
                    cluster_rows = FALSE,
                    cluster_columns = FALSE,show_column_names = FALSE,col = PurpleAndYellow(),
