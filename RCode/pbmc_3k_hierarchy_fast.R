...
###Load data
# original: pbmc.data <- Read10X(data.dir = "filtered_gene_bc_matrix") 
pbmc.data <- Read10X(data.dir = "ketogenic_dataset", gene.column = 1)

#####################################################################################################
###Data preprocessing with Seurat
pbmc <- CreateSeuratObject(counts = pbmc.data, project = "pbmc3k", min.cells = 3, min.features = 200)
pbmc[["percent.mt"]] <- PercentageFeatureSet(pbmc, pattern = "^MT-")
pbmc <- subset(pbmc, subset = nFeature_RNA > 200 & nFeature_RNA < 6000 & nCount_RNA < 30000 & percent.mt < 20)
...
# original new.cluster.ids <- c("Naive CD4 T", "CD14+ Mono", "Memory CD4 T", "B", "CD8 T", "FCGR3A+ Mono", "NK", "DC", "Platelet")
#new.cluster.ids <- c("Enterocyte", "Stem", "Goblet", "TransitAmplifying", "Tuft", "Enteroendocrine",
#                     "Paneth")
...
pbmc.markers <- FindAllMarkers(pbmc, only.pos = TRUE, min.pct = pct_test, logfc.threshold = 0.25)
###Select top10 genes for each cluster
top10 <- pbmc.markers %>% group_by(cluster) %>% top_n(n = n_test, wt = avg_log2FC)
...
i = 2
while(i <= length(pbmc_3k_hie_list)){
  PBMC.markers <- FindAllMarkers(pbmc_3k_hie_list[[i]], only.pos = TRUE, min.pct = pct_test, logfc.threshold = 0.25)
  top10 <- PBMC.markers %>% group_by(cluster) %>% top_n(n = n_test, wt = avg_log2FC)
...
for(i in seq_along(hierarchy_list)){
  for(j in seq_along(hierarchy_list[[i]])){
    if(length(str_split_1(hierarchy_list[[i]][[j]],";"))==2){
      ...
      PBMC.markers <- FindAllMarkers(PBMC_sub, only.pos = TRUE, min.pct = pct_test, logfc.threshold = 0.25)
      top10 <- PBMC.markers %>% group_by(cluster) %>% top_n(n = n_test, wt = avg_log2FC)
...
write.table(as.data.frame(hierarchy_list),file="activin_hierarchy_list.csv", quote=F,sep=",",row.names=F,col.names = F)
write.table(as.data.frame(hierarchy_gene_list),file="activin_hierarchy_gene_list.csv", quote=F,sep=",",row.names=F,col.names = F)
