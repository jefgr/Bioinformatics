...
This is not good, because diagonal blocks represent marker genes strongly expressed in their intended cluster.
...
The final result is a new heatmap (Figure~\ref{fig:figure1}), where the expression of the marker gene is more specific to the appropriate cluster. 
...
They compared $k=1$, $k=5$ and $k=7$, and they observed that these three values produced the same hierarchy as the one shown in the Figure ~\ref{fig:figure1}.
...
The merge is kept only if it reduces the undesirable off-diagonal signal. 
...
We know that the methods works and improves the marker genes selection problem, but we want to look whether its results are stable when marker selection parameters are modified.
...
If the selected genes change substantially when parameters change, then the biological interpretation of a branch or cell type is less stable than the tree visualisation suggests.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=1\linewidth]{additional_dataset_plot_hierarchy.png}
...
Our analysis does not show that the Sun and Qiu method fails. The broad hierarchy is stable in PBMC3k, and several marker gene sets classify cell types well.
The more precise conclusion is that the method can be stable at one level and less stable at another.
A method can be robust at the hierarchy level, acceptable at the classification level and still unstable at the marker gene level.

This distinction is important for biological interpretation.
If the goal is only to classify cells, several gene lists may be sufficient.
However, if the goal is to claim that specific genes characterise a lineage or cell type, then gene list stability becomes essential.
The original article emphasises interpretability, but interpretability should include not only a clean heatmap but also the stability and compactness of the genes selected.

The article of  Liu et al. argues that marker gene selection should balance classification performance with gene set size, because compact gene sets are easier to interpret and validate experimentally \cite{liu2026}.
Our results point in the same direction.
The largest gene set gives the highest mean KNN accuracy, but it may be less useful as a compact biological signature.
Conversely, a smaller gene set is easier to interpret but can lose predictive power.

The main weakness of the original article is therefore not that its results are false.
The central idea is sound, and the PBMC results support it.
The weakness is that the evaluation relies too heavily on heatmap clarity and classification accuracy.
These criteria are useful, but they do not fully test whether the same biological explanation is recovered across reasonable parameter choices.

There are still some limitations in our analysis: Jaccard similarity treats all marker genes equally.
It does not distinguish highly ranked markers from lower-ranked markers, nor does it evaluate whether changing genes affects biological pathways.
Additionally, KNN accuracy is only one downstream metric.
Other classifiers or cross-dataset mapping experiments could give a more complete evaluation.
